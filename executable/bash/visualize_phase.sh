...
then
  echo "$1"
  $python $visualizer --file_path "$1" --save_folder $save_folder --mode $mode --dpi $dpi --figsize $figsize_width $figsize_height --px_size $px_size --cmap $cmap --show_plot $show_plot --save_plot $save_plot --phase_ylabel $phase_ylabel
fi
