...
# TIE
solver = FFTSolver2D([intensity_minus_z, intensity_plus_z], DISTANCE, WAVELENGTH, PX_SIZE)
...
ax1.set_title(f'dz = {units.m2mm(minus_distance):.3f} mm')
ax3.set_title(f'dz = {units.m2mm(plus_distance):.3f} mm')
...
fig.tight_layout()
path = os.path.join(folder, f'error dz = {units.m2mm(DISTANCE):.3f} mm.png')
fig.savefig(path)
